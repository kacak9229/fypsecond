...
module.exports = function(app) {
  ...
  .post(function(req, res, next) {
    async.parallel([
      ...
      app.get('/student/certificates', function(req, res, next) {

        Certificate.find({ ownBy: req.user._id })
          .populate('ownBy course')
          .exec(function(err, certs) {
            console.log(certs)
            res.render('student/certificate', { certs: certs });
          });
      });

      app.get('/student/single-certificate/:id', function(req, res, next) {

        Certificate.findOne({ _id: req.params.id })
          .populate('ownBy course')
          .exec(function(err, cert) {
            console.log(cert)
            res.render('student/single-certificate', { cert: cert });
          });
      });
